...
# Function to generate audio with TTS based on emotions
def generate_audio(text, emotions):
    base_audio = gTTS(text=text, lang="en")
    base_audio.save("base_audio.mp3")

    # Emphasized text (simulate different emotional tones)
    emphasized_text = ""
    for sentence, emotion in emotions:
        if emotion == "Positive":
            emphasized_text += f"<emphasis level='strong'>{sentence}</emphasis> "
        elif emotion == "Negative":
            emphasized_text += f"<emphasis level='reduced'>{sentence}</emphasis> "
        else:
            emphasized_text += f"{sentence} "
...
if uploaded_file is not None:
    ...
    base_audio_path, emphasized_audio_path = generate_audio(text, emotional_words)

    st.header("Read PDF")
    if st.button("Play Base Audio"):
        st.audio(base_audio_path, format="audio/mp3")

    if st.button("Play Emphasized Audio"):
